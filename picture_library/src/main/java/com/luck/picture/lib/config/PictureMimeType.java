...
public final class PictureMimeType {
    ...
    public static String fileToType(File file) {
        if (file != null) {
           return pathToType(file.getPath());
        }
        return "image/jpeg";
    }

    /**
     * 根据文件路径（本地路径或网络路径）判断文件类型是图片还是视频
     * @param path
     * @return
     */
    public static String pathToType(String path){
        if (path.endsWith(".mp4") || path.endsWith(".avi")
                || path.endsWith(".3gpp") || path.endsWith(".3gp") || path.endsWith(".mov")) {
            return "video/mp4";
            ...
            return "image/jpeg";
                ...
                || path.endsWith(".flac") || path.endsWith(".lamr")|| path.endsWith(".voice")) {
            return "audio/mpeg";
        }
        return "image/jpeg";
    }
