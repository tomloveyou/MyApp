...
public abstract class NetworkBoundResource<ResultType, RequestType> {
    ...
    private void init() {
        ...
        result.addSource(dbSource, data -> {
            ...
            if (shouldFetch(data)) {
                ...
                result.addSource(dbSource, newData -> setValue(Result.success(newData)));
            }
        });
    }
    ...
    private void fetchFromNetwork(final LiveData<ResultType> dbSource) {
        ...
        // 先从数据库中获取数据
        result.addSource(dbSource, newData -> setValue(Result.loading(newData)));
                ...
                if (response instanceof Result) {
                    ...
                    if (code != NetConstant.REQUEST_SUCCESS_CODE) {
                        ...
                        result.addSource(dbSource,
                                newData -> setValue(Result.error(code, newData)));
                        return;
                    } else {
                        // do nothing
                    }
                ...
                threadManager.runOnWorkThread(() -> {
                    ...
                    threadManager.runOnUIThread(() ->
                            ...
                            result.addSource(safeLoadFromDb(),
                                    newData -> setValue(Result.success(newData)))
                    );
                });
            } else {
                onFetchFailed();
                result.addSource(dbSource,
                        newData -> setValue(Result.error(ErrorCode.API_ERR_OTHER.getCode(), newData)));
            }
        });
    }
